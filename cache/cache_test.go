...
func TestRequiredCapacity(t *testing.T) {
	...
	assert.Nil(t, cache, "Expected cache.New to return a nil LRUCache pointer if the capacity is 0")
	assert.Error(t, err, "Expected cache.New to return an error if the capacity is 0")
}
